...
Integrates kinetic/empirical yield models or ML surrogate models with thermal duty calculations,
...
class ReactorConfig:
    ...
    Attributes:
        ...
        yield_mode: Yield calculation engine mode ("DETERMINISTIC" | "ML_SURROGATE").
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.yield_mode not in ["DETERMINISTIC", "ML_SURROGATE"]:
            raise ValueError(f"Unknown yield_mode '{self.yield_mode}'. Must be 'DETERMINISTIC' or 'ML_SURROGATE'.")
...
class ReactorOutput:
    ...
    yield_engine_used: str = "DETERMINISTIC"
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "yield_engine_used": self.yield_engine_used,
...
class PyrolysisReactor:
    ...
    def process(
        ...
        yield_mode_override: Optional[str] = None,
    ) -> ReactorOutput:
        ...
        Args:
            ...
            yield_mode_override: Optional override for yield engine ("DETERMINISTIC" vs "ML_SURROGATE").
        ...
        yield_mode = (yield_mode_override or self.config.yield_mode).upper()
        ...
        # 2. Yield calculation via deterministic kinetic model or ML surrogate
        if yield_mode == "ML_SURROGATE" and self.ml_yield_predictor is not None:
            ml_res = self.ml_yield_predictor.predict_feedstock_yields(
                feedstock=feedstock,
                reactor_temp_c=temp_c,
                heating_rate_c_min=hr_c_min,
                residence_time_min=res_time,
                feed_rate_wet_kg_h=dried_feed_rate_kg_h,
                enforce_physics=True,
            )
            dry_yields = YieldFractions(
                biochar_yield=ml_res["biochar_yield"],
                bio_oil_yield=ml_res["bio_oil_yield"],
                syngas_yield=ml_res["syngas_yield"],
            )
            daf_yields = dry_yields
            engine_tag = "ML_SURROGATE"
        else:
        ...
        q_sensible_total_kj_h = q_sens_dry_solid + q_sens_residual_water + q_carrier
        q_direct_total_kj_h = q_sensible_total_kj_h + q_reaction

        q_loss_kj_h = q_direct_total_kj_h * (self.config.heat_loss_fraction / (1.0 - self.config.heat_loss_fraction))
        q_gross_total_kj_h = q_direct_total_kj_h + q_loss_kj_h

        q_gross_kw = q_gross_total_kj_h / 3600.0
        q_gross_mj_h = q_gross_total_kj_h / 1000.0

        # 5. Product Heating Values (HHV / LHV)
        lhv_feed = feedstock.calculate_lhv_dry()
        hhv_char = min(32.0, max(18.0, 1.25 * lhv_feed * (1.0 - 0.0003 * (temp_c - 400.0))))
        hhv_dry_oil = min(26.0, max(14.0, 0.95 * lhv_feed))
        lhv_syngas = min(18.0, max(6.0, 6.0 + 0.015 * (temp_c - 350.0)))
        ...
        return ReactorOutput(
            ...
            reactor_thermal_duty_kw=q_gross_kw,
            reactor_thermal_duty_mj_h=q_gross_mj_h,
            sensible_heating_duty_kw=q_sensible_total_kj_h / 3600.0,
            reaction_duty_kw=q_reaction / 3600.0,
            heat_loss_kw=q_loss_kj_h / 3600.0,
            operating_temperature_c=temp_c,
            char_hhv_mj_kg=hhv_char,
            dry_bio_oil_hhv_mj_kg=hhv_dry_oil,
            syngas_lhv_mj_kg=lhv_syngas,
            yield_engine_used=engine_tag,
            assumptions={
